    ...
    */
    money.sendUserInput = function(userId, input, callback, bypass) {
        ...
        if (!bypass) {
            ...
            var types = ['speech', 'text', 'multimedia', 'touch', 'blob'];
          ...
          if (callback) {
            money.getRecommendations(userId, options, function(err, res) {
              if (err) {
                ...
                callback(err, facebook.carousel(res, options ? options.label : null));
              }
            });
          } else {
            return money.getRecommendations(userId, options)
              .then(function(res) {
                return facebook.carousel(res, options ? options.label : null);
              });
          }
        },
          ...
          if (callback) {
            money.sendUserInput(userId, input, function(err, res) {
              if (err) {
                ...
                callback(err, facebook.carousel(res, input.label || null));
              } else {
                callback(err, res);
              }
            }, 'facebook');
            ...
            return money.sendUserInput(userId, input, null, 'facebook')
              .then(function(res) {
                if (res !== 'OK') {
                  return facebook.carousel(res, input.label || null);
                }
                return res;
              });
          }
